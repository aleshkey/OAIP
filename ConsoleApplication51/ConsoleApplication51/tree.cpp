...
void Delete(int x, node* (&root))
{
	...
	else if (x < root->info) //идем в левое поддерево 
	{
		Delete(x, root->left);
	}
	else if (x > root->info) //идем в правое поддерево 
	{
		Delete(x, root->right);
	}
	else if ((root->left == NULL) && (root->right == NULL)) //если лист 
	{
		d = root;
	...
	else if (root->left == NULL) //если есть правое поддерево
	{
		d = root;
		root = root->right;
		free(d);
	...
	else if (root->right == NULL) // если есть левое поддерево 
	{
		d = root;
		root = root->left;
		free(d);
	...
	else // если есть и левое, и правое поддерево
	{
...
void Clear(node* (&p)) //функция очистки дерева 
{
	if (p != NULL)
	{
		Clear(p->left);
...
{
	if (root->info < value)// если root меньше, чем value
		return Search(root->right, value);
	else if (root->info > value)// если root больше, чем value
...
void View_Tree(node* p, int level) {
	...
	if (p) {
		...
		str += to_string(p->info); // перевод из int в string  
		cout << str << endl;
