...
public:
	Graph(long n, long m, long k, vector<long> setsSystem) {
		this->n = n;
		this->m = m;
		this->k = k;
		this->setsSystem = setsSystem;
...
long main()
{
	...
	Graph graph(n, m, k, setsSystem);
	long from[3], to[3];
	long mainIndex = 0;
	for (long i = 0; i < t; ++i)
	{
		fscanf(fin, "%d%d%d%d%d%d", &from[0], &from[1], &from[2], &to[0], &to[1], &to[2]);
