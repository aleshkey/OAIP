...
int** Dop(int** A, int** B, int mA, int mB);
int main()
{
	...
	//menu();
	cout << "Введите мощность области отправления соответствия A: " << endl;
	int n1;
	cin >> n1;
	int* X_1 = new int[n1];
	...
	int* X = Z(X_1[0], X_1[n1 - 1]);
	cout << "Введите мощность области прибытия соответствия A: " << endl;
	int m1;
	cin >> m1;
	int* Y_1 = new int[m1];
	cout << "Введите элементы множества: " << endl;
	...
	int* Y = Z(Y_1[0], Y_1[m1 - 1]);

	cout << "Введите мощность графика F соответствия A:\t";
	int mG;	//  Пользователь вводит мощность n графика  G  первого соответствия A.
	cin >> mG;
	int** G = PereChislenie(mG);	//  Пользователь последовательно вводит n пар графика G соответствия A.
	cout << "Введите мощность области отправления соответствия B: " << endl;
	int n2;
	cin >> n2;
	int* U_1 = new int[n2];
	cout << "Введите элементы множества: " << endl;
	...
	int* U = Z(U_1[0], U_1[n2 - 1]);
	cout << "Введите мощность области прибытия соответствия B: " << endl;
	int m2;
	cin >> m2;
	int* V_1 = new int[m2];
	cout << "Введите элементы множества: " << endl;
	...
	int* V = Z(V_1[0], V_1[m2 - 1]);
	cout << "Введите мощность графика G соответствия B:\t";
	int mF;
	cin >> mF;	
	int** F = PereChislenie(mF);	
	KonZap();	
	...
	while (true)
	{
		...
		menu();
		int Operaziya; //пользователь выбирает операцию 
		cin >> Operaziya;
		system("cls");
		cout << "А = <";	WatchMn(X, X[0]); cout << ", "; WatchMn(Y, Y[0]); cout << ", "; Watch(G, mG); cout << ">" << endl;
		cout << "В = <";	WatchMn(U, U[0]); cout << ", "; WatchMn(V, V[0]); cout << ", "; Watch(F, mF);  cout << ">" << endl << endl;
		switch (Operaziya)
		{
		case 1: 
		{
			...
			for (int i = 0; i <= mD; i++)
				D[i] = new int[3];
			D = Peresek(G, F, mG, mF);	//  Нахождение пересечения графиков G и F:
			mD = D[0][0];
			int* H = new int[X[0] + U[0]];
			H = PeresekMn(X, U, X[0], U[0]);	//   Нахождение пересечения множеств X и U:
			int* J = new int[Y[0] + V[0]];
			J = PeresekMn(Y, V, Y[0], V[0]);	// Нахождение пересечения множеств Y и V:
			cout << "<"; WatchMn(H, H[0]); cout << ", "; WatchMn(J, J[0]); cout << ", "; Watch(D, mD); cout << ">" << endl;	//  Выводим соответствие < H, J, C > на экран.
			system("pause");
			break;
		}
		case 2: //  Нахождение объединения соответствий 
		{
			...
			for (int i = 0; i <= mC; i++)
				C[i] = new int[3];
			C = InOne(G, F, mG, mF);	// Нахождение объединения графиков G и F:
			mC = C[0][0];
			int* K = new int[X[0] + U[0]];
			K = InOneMn(X, U, X[0], U[0]);	// Нахождение объединения множеств X и U:
			int* L = new int[Y[0] + V[0]];
			L = InOneMn(Y, V, Y[0], V[0]);	// Нахождение объединения множеств Y и V:
			cout << "<"; WatchMn(K, K[0]); cout << ", "; WatchMn(L, L[0]); cout << ", "; Watch(C, mC); cout << ">" << endl;	//   Выводим на экран соответствие <K,L, D >.
			system("pause");
			break;
		}
		
		case 3: //  27  Нахождение разности соответствий 
		{
		...
		case 4: //  28  Нахождение разности соответствий B и A:
		{
			...
			for (int i = 0; i <= mR; i++)
				R[i] = new int[3];
			R = Pa3HocTb(F, G, mF, mG);	//   Нахождение разности графиков F и G:
			mR = R[0][0];
			int* T = new int[U[0] + 1];
			T = Pa3HocTbMn(U, X, U[0], X[0]);	//   Нахождение разности множеств U и X:
			int* Q = new int[V[0] + 1];
			Q = Pa3HocTbMn(V, Y, V[0], Y[0]);	//  Нахождение разности множеств V и Y:
			cout << "<"; WatchMn(T, T[0]); cout << ", "; WatchMn(Q, Q[0]); cout << ", "; Watch(R, mR); cout << ">" << endl;//  Выводим соответствие < T,Q, R>  на экран.
			system("pause");
		}
		break;
		case 7: //  Нахождение инверсии соответствия A: 
		{
		...
		break;
		case 8: //  30  Нахождение инверсии соответствия B:
		{
			...
			for (int i = 0; i <= mI; i++)
				I[i] = new int[3];
			I = Inversion(F, mF);	//  Нахождение инверсии графика F:
			mI = I[0][0];
			cout << "<"; WatchMn(V, V[0]); cout << ", "; WatchMn(U, U[0]); cout << ", "; Watch(I, mI); cout << ">" << endl;	// 30.5  Выводим соответствие <V,U,I> на экран.
			system("pause");
		}
		break;
		case 5: //  Нахождение симметрической разности соответствий A и B:
		{
			...
			for (int i = 0; i <= mS; i++)
				S[i] = new int[3];
			S = SimRas(F, G, mF, mG);	//  Нахождение симметрической разности графиков G и F :
			mS = S[0][0];
			int* O = new int[X[0] + U[0] + 2];
			O = SimRasMn(X, U, X[0], U[0]);		//  Нахождение симметрической разности множеств X и U:
			int* H = new int[Y[0] + V[0] + 2];
			H = SimRasMn(Y, V, Y[0], V[0]);		//  Нахождение симметрической разности множеств Y и V:
			cout << "<"; WatchMn(O, O[0]); cout << ", "; WatchMn(H, H[0]); cout << ", "; Watch(S, mS); cout << ">" << endl;	// 31.5  Выводим соответствие <O,H,C > на экран
			system("pause");
		}
		break;
		case 9: //  Нахождение композиции соответствий A и B:
		{
			...
			for (int i = 0; i <= mK; i++)
				K[i] = new int[3];
			K = Exposition(G, F, mG, mF);	//  Нахождение композиции графиков G и F :
			mK = K[0][0];
		...
		case 6:
		{
			cout << "Дополнение соответствия A до универсума : " << endl;
			int mK = X[0] * Y[0] + 1;
			int* un_1 = InOneMn(X, U, X[0], V[0]);
			int* un_2 = InOneMn(Y, V, Y[0], V[0]);
			int* r_1 = Pa3HocTbMn(un_1, X, un_1[0], X[0]);
			int* r_2 = Pa3HocTbMn(un_2, Y, un_2[0], Y[0]);
			int mC = mG + mF + 1;
			int** C = new int*[mC];
			for (int i = 0; i <= mC; i++)
				C[i] = new int[3];
			C = Dop(F, G, mF, mG);	//  Нахождение объединения графиков G и F:
			mC = C[0][0];
			cout << "<"; WatchMn(r_1, r_1[0] - 1); cout << ", "; WatchMn(r_2, r_2[0]); cout << ", "; Watch(C, mC); cout << ">" << endl;	// 26.5  Выводим на экран соответствие <K,L, D >.
			system("pause");
		}

		case 10: //  Нахождение композиции соответствий B и A:
		{
		...
		break;
		case 11://  34  Нахождение образа соответствия A:
		{
			int mR = (Y[0] * mG / 2);
			int* R = new int[mR];	// 34.3  Создаем пустое множество M.
		...
		break;
		case 12://  35  Нахождение прообраза соответствия A:
		{
			int mZ = (Y[0] * mG);
		...
		//}
		//break;
		case 13://  38  Нахождение сужения соответствия A: 
		{
			int mE = mG + Y[0] * Y[0];
			int** E = new int*[mE];
			for (int i = 1; i <= mE; i++)
		...
		break;
		case 14://  39  Нахождение продолжения соответствия A:
		{
			cout << "Продолжение А:\n";
			int mJ = mG + 2;
...
void  menu()
{
	...
	cout << "	1.  Пересечение соответствий.\n	2.  Объединение соответствий.\n	3.  Разность соответствий A и В.\n 	3.  Разность соответствий В и А\n       5.	Симметрическая разность соответствий.\n	 6.   Дополнение до универсума соответствия Г1\n 7.  Инверсия соответствия А.\n";
	cout << "	\n8. инверсия соответсвия В \n	9.   Композиция соответствий А и В.\n 10.   Композиция соответствий В и А.\n ";
	cout << "	11.  Образ А.\n 12. Прообраз А.\n	\n	10. Cужение А.\n";
	cout << "	11.  проделжение\n";
	cout << "-->";
}
int** Dop(int** A, int** B, int mA, int mB)
{
	int** res = InOne(A, B, mA, mB);
	int** res_2 = Pa3HocTb(res, A, res[0][0], mA);
	return res_2;
